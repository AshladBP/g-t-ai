...
class ActorNetwork(nn.Module):
    def __init__(self, nb_actions, input_dims, alpha, fc1_dims=256, fc2_dims=256, chkpt_dir='models'):
...
class Agent:
    ...
    def load_actor_model(self, file_path):
        ...
        self.actor.load_state_dict(T.load(file_path))

    def load_critic_model(self, file_path):
        print("loading critic model...")
        self.critic.load_state_dict(T.load(file_path))

    def get_model_data(self):
        actor_buffer = io.BytesIO()
        critic_buffer = io.BytesIO()
        
        T.save(self.actor.state_dict(), actor_buffer)
        T.save(self.critic.state_dict(), critic_buffer)
        
        actor_buffer.seek(0)
        critic_buffer.seek(0)
        
        return actor_buffer.getvalue(), critic_buffer.getvalue()
